...
// Default injector template — 5 events showing a trajectory with an anomaly spike
...
export default function DashboardPage() {
  ...
  const runDetection = useCallback(async () => {
    ...
    try {
      ...
      if (mode === "injector") {
        // Parse and validate the injector JSON
        let parsed: unknown;
        try {
        ...
        body = { telemetry: parsed };
      } else {
        body = { scenario, count: 30, tourist_count: 5 };
      }
        ...
        {mode === "injector" && (
          <div className="glass rounded-xl mb-6 overflow-hidden">
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-violet-400/60 font-mono hidden sm:inline">
                  real-time ready
                </span>
              ...
              {injectorOpen && (
                ...
                >
                  <div className="px-4 md:px-6 pb-4 md:pb-6 border-t border-white/[0.06] pt-4">
                    {/* Schema hint */}
                    <div className="flex items-start gap-2 mb-3 p-3 rounded-lg bg-violet-500/5 border border-violet-500/10">
                      <Code2 className="w-3.5 h-3.5 text-violet-400 mt-0.5 flex-shrink-0" />
                      <div className="text-[11px] text-zinc-500 leading-relaxed">
                        ...
                        <span className="block mt-1 text-zinc-600">Min 2 events required. In the future, this input will accept a WebSocket or API stream for live data.</span>
                      </div>
                    </div>
                    ...
                    <div className="relative">
                      <textarea
                        ...
                        spellCheck={false}
                        className="w-full h-64 bg-[#0c0c0e] border border-white/[0.06] rounded-lg p-4 text-xs font-mono text-emerald-400/80 leading-relaxed resize-y focus:outline-none focus:border-indigo-500/30 focus:ring-1 focus:ring-indigo-500/20 placeholder-zinc-700"
                    ...
                    <div className="flex items-center gap-3 mt-3">
                      <button
                        onClick={runDetection}
                        disabled={loading}
                        className="inline-flex items-center gap-2 px-5 py-2.5 bg-violet-500 hover:bg-violet-400 disabled:bg-violet-500/50 text-white text-sm font-medium rounded-lg transition-all hover:shadow-lg hover:shadow-violet-500/25 disabled:cursor-not-allowed"
                      >
                        {loading ? (
                          <>
                            <Loader2 className="w-4 h-4 animate-spin" />
